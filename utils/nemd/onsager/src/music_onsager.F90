...
Program onsager
  ...
  Real(kind=RDbl) :: L_x, L_y, L_z, L_tot, prefac, integ_err
  ...
  Do i=1,nsorbs
    Do j=1,nsorbs
      ...
      integ_err=mmath_simpson(fc(i,j,n_fc/2:n_fc),deltaT,n_fc/2)
      L_x=x_integ*prefac
      L_y=y_integ*prefac
      L_z=z_integ*prefac
      L_tot=tot_integ*prefac/3
      Write(*,'(a,2i2,4e11.3,f10.3)') &
          "i,j,Lx,Ly,Lz,LTot Err%: ",i,j,L_x, L_y, L_z, L_tot, &
          100*Abs(integ_err/tot_integ)
    end do
  End Do
