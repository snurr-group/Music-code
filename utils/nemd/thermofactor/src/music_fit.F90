...
Program fitting
  ...
  Real(kind=RDbl) :: P, tolerance, mserror,b
  ...
  Write(outunit,*) "%Filename   y     K     Lmax,    a, b ,error"
  Write(outunit,*) "%Number points : ", nisoT
  ...
  Do i=1,nisoT
    ...
    Call isotherm_guess(isos(i),K,Lmax,a,b,PMax)

    Write(*,*) "Guesses are : ", K, Lmax, a, b, PMax
    mserror=isotherm_MSerror(isos(i),K, Lmax, a, b, PMax)
    Write(*,*) "Error with this Guess : ", mserror
    ! WRONG, Eqn is N = K*P / ( 1+ (K/Lmax)*P + a * (P/PMax)**2)
    ! where K, Lmax and a are adjustable
    ! at the end K, Lmax and a/Pmax^2 will be reported

    Call isotherm_fit(isos(i), K, Lmax, a, b, PMax, tolerance)
    mserror=isotherm_MSerror(isos(i),K, Lmax, a, b, PMax)
    ! write to outpu tfile
    Write(outunit,'(a,t10,f10.4,1x,e12.3,1x,2f10.5,1x,2e12.3)') &
        Trim(isofile(i)), gas_y(i), K, Lmax, a, b, mserror 
