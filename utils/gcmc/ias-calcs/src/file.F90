...
Contains
  ...
  !----------------------------------------------------------------------------
!!$  Character(len=strLen) Function file_getname(unit)
  ...
  ! fileopt numbers are:
  ! nothing -- no assumptions about previous file status DANGEROUS!
  !   fileopt = IJK, 3 digit number (decimal system)
  !   If fileopt=0 default open statement
  !   If J=1 formatted file,          J=0  unformatted file
  !   If K=1 new file,                K=0  old file
  !   example:  fileopt = IJK = 101 ,-> J=0 unformatted, K=1 new
  !----------------------------------------------------------------------------
  Integer Function file_openRegular(infile,fileopt,tag)
    ...
    If (ios/=0) Then
      ...
      Write(0,*) 'Error while opening file: '//'"'//Trim(filename)//'"'
  ...
  ! openned by specifying the scratchNo option.
  !----------------------------------------------------------------------------
  Integer Function file_openScratch(recLen,scratchNo)
    Integer, Intent(In) :: recLen
    Integer, Intent(In), Optional :: scratchNo
    Integer :: error
    Character(len=strLen) :: filename,number

    !** Make up a fake name to get a unit number
    filename = "scratch"
    Write(number,'(i2)') scratchNo
    If (Present(scratchNo)) filename = Trim(filename)//Trim(number)
