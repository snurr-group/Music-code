...
Module fundcell
  ...
  Use defaults, Only: RDbl, strLen, RSgl, degTorad, Pi, lstrLen
...
Contains
  ...
  ! a, b, c                   # bounding box size  (RDbl)
  ! Requires: fcell -- fundamental cell type to be initialized
  !           unit -- unit to read from
  !           orthoEl -- optional orthogonal cell lengths
  ! If orthoEl is present, will assume the cell is orthogonal wo reading file
  !----------------------------------------------------------------------------
  Subroutine fundcell_init(fcell,unit,orthoEl)
    Type(Fundamental_Cell), Intent(OUT)   :: fcell
    Integer, Intent(In)                   :: unit
    Type(VecType), Intent(In), Optional   :: orthoEl

    Integer                   :: i
    Real(kind=RDbl)           :: anglebc,angleac,angleab,err

    If (Present(orthoEl)) Then
      !** The edge lengths have been passed. This means it is the special
      !** case of an orthorhombic cell
      fcell%ell(1) = orthoEl%comp(1)
      fcell%ell(2) = orthoEl%comp(2)
      fcell%ell(3) = orthoEl%comp(3)
      anglebc = 90.0_RSgl
      angleac = 90.0_RSgl
      angleab = 90.0_RSgl
      fcell%origin = VecType(0.0_RDbl)
      fcell%eff = orthoEl
    Else
      !** Read the cell parameters in from the given unit
      Read(unit,*) fcell%ell(1), fcell%ell(2), fcell%ell(3)
      Read(unit,*) anglebc, angleac, angleab 
      Read(unit,*) (fcell%origin%comp(i),i=1,3) 
      Read(unit,*) (fcell%eff(i),i=1,3) ! orthorhombic box lengths
    End If
    ...
    err = Abs(anglebc - 90.000) + Abs(angleac - 90.000) + Abs(angleab - 90.000)
    If (err > 0.001) Then
    ...
    !** convert to radians
    fcell%anglebc = anglebc*degTorad
    fcell%angleac = angleac*degTorad
    fcell%angleab = angleab*degTorad
...
!LC    Call fundcell_display(largecell,2,6)
  ...
  Subroutine fundcell_snglMinImage(fcell,atvec1,atvec2,sepvec,dist2)
    ...
    Do i = 1,3
      If (is_slanted) Then
        d = vec1%comp(i) - vec2%comp(i)
      Else
        d = atvec1%comp(i) - atvec2%comp(i)
      End If

      If (d > fcell%half_ell(i)) Then
        d = d - fcell%ell(i)
      Else If (d < - fcell%half_ell(i)) Then
        d = d + fcell%ell(i)
      End If
       sepvec%comp(i) = d
    End Do
    ...
    If (is_slanted) sepvec = fundcell_unslant(fcell,sepvec)
  ...
  Subroutine fundcell_multMinImage(fcell,atvecs1,atvecs2,sepvecs,dists2)
    ...
    Do a1 = 1, Size(atvecs1,1)
      Do a2 = 1, Size(atvecs2,1)
